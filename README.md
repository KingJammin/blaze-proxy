<p align="center">
  <img src="build/icon.svg" width="160" height="160" alt="Blaze Proxy logo">
</p>

<h1 align="center">Blaze Proxy</h1>

<p align="center">
  Route AI clients to your own models without changing how those clients work.
</p>

<p align="center">
  <a href="LICENSE"><img alt="License: MIT" src="https://img.shields.io/badge/license-MIT-f97316.svg"></a>
  <img alt="Node.js 20 or newer" src="https://img.shields.io/badge/node-%3E%3D20-101010.svg">
  <img alt="OpenAI-compatible" src="https://img.shields.io/badge/API-OpenAI--compatible-f97316.svg">
</p>

Blaze Proxy is a desktop AI model router. Point your AI tools at `http://127.0.0.1:8789/v1` and Blaze decides, per model, whether a request passes through to its real provider untouched or is intercepted and served by **your own endpoint**—a local GPU box, a homelab tunnel, or any OpenAI-compatible server.
...
Start the desktop app, or run the daemon headlessly:

```bash
blaze-proxy app       # desktop controls + live request tail
blaze-proxy start     # headless, foreground
```

In the app, open **Settings** and set your OpenAI-compatible endpoint. Then enable a model in the routing list—or turn on **Route all**—and point your client at:
...
Any client that accepts a custom base URL can use Blaze. This request is intercepted when its model rule—or **Route all**—is enabled:
...
curl -N http://127.0.0.1:8789/v1/chat/completions \
  ...
  -d '{
    ...
    "messages": [{"role": "user", "content": "Explain this repo in one sentence."}],
    "stream": true
  }'
```
...
client = OpenAI(base_url="http://127.0.0.1:8789/v1", api_key="blaze-local")

response = client.chat.completions.create(
    model="gpt-5.3-codex-spark",
    messages=[{"role": "user", "content": "Give me three release-name ideas."}],
)

print(response.choices[0].message.content)
```

The placeholder client key above is only needed by SDKs that require a value. Blaze uses the separately configured endpoint credential when it forwards an intercepted request.

### Minimal headless configuration

Blaze creates `~/.blaze-proxy/config.json` on first run. For a local server on port 8000, the smallest useful override is:

```json
{
  "endpoint": "http://127.0.0.1:8000/v1",
  "endpointAuth": { "type": "value", "value": "" },
  "routeAll": true
}
```

Config changes are hot-reloaded. Turn `routeAll` back off when you want only explicitly enabled model rules to be intercepted.

## How routing works

| Request | Rule | Result |
|---|---|---|
| Known model, routing enabled | `model.route: true` | Sent to your configured endpoint using the model's `dest` value |
| Any model, **Route all** enabled | `routeAll: true` | Sent to your configured endpoint |
| No matching route | — | Passed through to the original provider |
| WebSocket upgrade | — | Tunnelling only; the live tail labels an active-rule mismatch as **BYPASS** |

Use `blaze-proxy status` for a summary and the desktop app's live tail to see the decision made for each request.

## Client guides

- [Codex](#codex-use-a-custom-provider-or-routing-cant-reach-you)
- [Any OpenAI-compatible client](#openai-compatible-request)
- [Transparent mode on macOS](#transparent-mode-macos-opt-in-off-by-default)
